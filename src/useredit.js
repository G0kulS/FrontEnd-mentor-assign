...
export default  function Useredit()
{
    ...
    let [m,um] = useState("");
    useEffect( async ()=>{
        let temp = await axios.get(`https://nodementor-assign.herokuapp.com/student/${id}`);
        us(temp.data.name);
        ss(temp.data.subject);
        let name = await axios.get(`https://nodementor-assign.herokuapp.com/mentor/${temp.data.mentor_id}`);
        um(name.data.name);
    },[])
           ...
           <li class="list-group-item">
               <form onSubmit={
                 async  (e)=>{
                      ...
                      await axios.put(`https://nodementor-assign.herokuapp.com/student/${id}`,obj)
                      um("");
                   }
               }>
               <div class="mb-3">
      <label for="exampleFormControlInput1" class="form-label">you can change mentor</label>
