...
{     
     ...
     useEffect(async()=>{
       let temp =await axios.get("https://nodementor-assign.herokuapp.com/mentor");
       console.log(temp.data);
        umentor([...(temp.data)]);
        let tem = (await (axios.get("https://nodementor-assign.herokuapp.com/student"))).data;
        console.log("r",tem);
        ustudent([...tem])
     },[]);
    return <div class="container-fluid">
           ...
           <ul class="list-group">
            <li class="list-group-item active" aria-current="true">Mentor's List</li>
            {
              ...
              <Link to ={"/mentor/"+item.id}>  <button class="btn btn-secondary" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
                  ...
                  await axios.delete(`https://nodementor-assign.herokuapp.com/mentor/${item.id}`);
                  let temp =await axios.get("https://nodementor-assign.herokuapp.com/mentor");
                  console.log(temp.data);
                   umentor([...(temp.data)]);                       
                }}>Delete</button>
             ...
             {
              student.map((item)=>{
                return  item.mentor_id==undefined? 
           ...
           <Link to ={"/student/"+item.id} > <button class="btn btn-success" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
                  ...
                  await axios.delete(`https://nodementor-assign.herokuapp.com/student/${item.id}`);
                  let temp =await axios.get("https://nodementor-assign.herokuapp.com/student");
                  console.log(temp.data);
                   ustudent([...(temp.data)]);                       
                }}>Delete</button>
              </div></li>:<></>
